...
default_yaml = """subdomain_discovery:
    uses_tool: [ all ]
    thread: 10
    wordlist: default
...
port_scan:
    # currently bigRecon only supports naabu, nmap and other port scanning tools
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.INFO, "Scan Engine Added successfully"
            )
            return http.HttpResponseRedirect(reverse("scan_engine_index"))

    form.fields["yaml_configuration"].initial = default_yaml
...
def delete_engine(request, id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request, messages.INFO, "Oops! Engine could not be deleted!"
        )
    ...
    if request.method == "POST":
        if form.is_valid() and "upload_file" in request.FILES:
            ...
            if txt_file.content_type == "text/plain":
                ...
                wordlist_file = open(
                    wordlist_path + form.cleaned_data["short_name"] + ".txt", "w"
                )
                ...
                messages.add_message(
                    request,
                    messages.INFO,
                    "Wordlist " + form.cleaned_data["name"] + " added successfully",
                )
...
def delete_wordlist(request, id):
    ...
    if request.method == "POST":
        ...
        responseData = {"status": "false"}
        messages.add_message(
            request, messages.INFO, "Oops! Wordlist could not be deleted!"
        )
    return http.JsonResponse(responseData)
...
def configuration_list(request):
    ...
    context = {"configuration_nav_active": "true", "configurations": configurations}
...
def add_configuration(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            form.save()
            messages.add_message(
                request, messages.INFO, "Configuration added successfully"
            )
...
def delete_configuration(request, id):
    ...
    if request.method == "POST":
        ...
        responseData = {"status": "true"}
        messages.add_message(
            request, messages.INFO, "Configuration successfully deleted!"
        )
    else:
        responseData = {"status": "false"}
        messages.add_message(
            request, messages.INFO, "Oops! Configuration could not be deleted!"
        )
    return http.JsonResponse(responseData)
...
def update_configuration(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(
                request, messages.INFO, "Configuration edited successfully"
            )
            return http.HttpResponseRedirect(reverse("configuration_list"))
    else:
        form.set_value(configuration)
    context = {"configuration_nav_active": "true", "form": form}
