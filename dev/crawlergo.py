...
def process_data(id,threadName, q):
    while not exitFlag:
        if q.empty() == False:
            ...
            cmd = [crawlergo_path, "-c", chrome_path,"-t", "10","-f","smart","--fuzz-path", "--output-mode", "json","--ignore-url-keywords", "quit,exit,logout",  "--custom-headers", simplejson.dumps(headers),"--robots-path","--log-level","debug","--push-pool-max", "10", "--push-to-proxy","http://127.0.0.1:8888/",target]
