...
def test(request):
    context = {}
    return render(request, "mto.zing.vn_2020_11_20_14_39_34/run-xray.html", context)
...
def detail_scan(request, id=None):
    if id:
        ...
        alive_count = (
            ScannedHost.objects.filter(scan_history__id=id)
...
def start_scan_ui(request, host_id):
    ...
    if request.method == "POST":
        # get engine type
        print("chayjh di con")
        print("chay nekkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
        engine_type = request.POST["scan_mode"]
        scan_history_id = create_scan_object(host_id, engine_type)
        # start the celery task
        celery_task = doScan.apply_async(args=(host_id, scan_history_id, 0, None))
        ScanHistory.objects.filter(id=scan_history_id).update(celery_id=celery_task.id)
        messages.add_message(
            request, messages.INFO, "Scan Started for " + domain.domain_name
        )
...
def export_subdomains(request, scan_id):
    ...
    response["Content-Disposition"] = (
        'attachment; filename="subdomains_'
        + domain_results.domain_name.domain_name
        + "_"
        + str(domain_results.last_scan_date.date())
        + '.txt"'
    )
    return response
...
def export_endpoints(request, scan_id):
    ...
    response = HttpResponse(response_body, content_type="text/plain")
    response["Content-Disposition"] = (
        'attachment; filename="endpoints_'
        + domain_results.domain_name.domain_name
        + "_"
        + str(domain_results.last_scan_date.date())
        + '.txt"'
...
def schedule_scan(request, host_id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request, messages.INFO, "Scan Scheduled for " + domain.domain_name
        )
