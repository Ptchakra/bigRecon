...
def delete_target(request, id):
    ...
    if request.method == "POST":
        os.system(
            "rm -rf " + settings.TOOL_LOCATION + "scan_results/" + obj.domain_name + "*"
        )
        obj.delete()
        ...
        messages.add_message(
            request, messages.INFO, "Oops! Domain could not be deleted!"
        )
