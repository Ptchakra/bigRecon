...
def index(request):
    ...
    recent_completed_scans = (
        ...
        .filter(Q(scan_status=0) | Q(scan_status=2) | Q(scan_status=3))[:4]
    )
    currently_scanning = ScanHistory.objects.order_by("-last_scan_date").filter(
        scan_status=1
    )[:4]
    ...
    vulnerability_feed = VulnerabilityScan.objects.all().order_by("-discovered_date")[
        :20
    ]
    print("view 1")
    ...
    most_vulnerable_target = (
        VulnerabilityScan.objects.values("url")
        .annotate(num_vul=Count("url"))
        .order_by("-num_vul")[:7]
    )
    print(most_vulnerable_target)
    print("views1-2")
    most_common_vulnerability = (
        VulnerabilityScan.objects.values("name", "severity")
        .exclude(severity=0)
        .annotate(count=Count("name"))
        .order_by("-count")[:7]
    )
    print(most_common_vulnerability)
    print("views1-3")
    last_week = timezone.now() - timedelta(days=7)

    print("views 2")
    count_targets_by_date = (
        Domain.objects.filter(insert_date__gte=last_week)
    ...
    count_subdomains_by_date = (
        ScannedHost.objects.filter(discovered_date__gte=last_week)
    ...
    count_vulns_by_date = (
        VulnerabilityScan.objects.filter(discovered_date__gte=last_week)
        .annotate(date=TruncDay("discovered_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("-date")
    ...
    count_scans_by_date = (
        ScanHistory.objects.filter(last_scan_date__gte=last_week)
        .annotate(date=TruncDay("last_scan_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("-date")
    )
    count_endpoints_by_date = (
        WayBackEndPoint.objects.filter(discovered_date__gte=last_week)
        .annotate(date=TruncDay("discovered_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("-date")
...
def on_user_logged_out(sender, request, **kwargs):
    messages.add_message(
        request,
        messages.INFO,
        "You have been successfully logged out. Thank you for using bigRecon.",
    )
...
def on_user_logged_in(sender, request, **kwargs):
    messages.add_message(
        request, messages.INFO, "Hi @" + request.user.username + " welcome back!"
    )
